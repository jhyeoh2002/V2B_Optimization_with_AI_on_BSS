...
class GurobiOptimizer:
    ...
    def optimize(self, availability, t_dep, soc_arr, soc_dep, bldg, elec_G2B, elec_G2V, pv):
        ...
        # Check for NaNs in PV data (The culprit)
        if np.isnan(pv).any():
            print(f"Warning: Found {np.isnan(pv).sum()} NaN values in PV data {pv}. Replacing with 0.0.")
        ...
        # 2. Suppress all output (logs and parameter changes)
        env.setParam("OutputFlag", 0)

        # 3. Start the environment
        env.start()

        # 4. Create the model using this specific environment
        # Create a new Gurobi model
        model = gp.Model("V2B_Optimization", env=env)

        # Set model parameters
        model.setParam("Threads", 1)  # Use 1 thread for parallel optimization
        model.setParam("MIPGap", 0.005)  # Set a 0.5% MIP gap
        model.setParam("outputflag", 0)  # Enable output logging
