...
def get_battery_details(window_length = 30):
    ...
    available = np.load(f"/home/lisa4090/Documents/GitHub/V2B_Optimization_with_AI_on_BSS/data/processed/battery_schedule_window{window_length}.npy")
    available = [sample[~np.isnan(sample).any(axis=1)].astype(int) for sample in available]

    details = np.load(f"/home/lisa4090/Documents/GitHub/V2B_Optimization_with_AI_on_BSS/data/processed/battery_details_window{window_length}.npy")

    index = np.load(f"/home/lisa4090/Documents/GitHub/V2B_Optimization_with_AI_on_BSS/data/processed/battery_series_window{window_length}.npy")[:,0].astype(int)
